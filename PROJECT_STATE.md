...
- **A real-device check by the user surfaced a second, deeper bug**: booking with a photo on staging
  ...
  - **Not yet confirmed**: whether the container restart to pick up the corrected compose file has
    actually happened — the sandbox blocked the restart command itself (`docker compose up -d`) after
    allowing the file write, so this depends on either the user running it directly or the next `dev`
    push re-triggering `deploy-dev.yml`, which will apply it automatically.
- **Still not done**: an actual end-to-end booking-with-photo-through-to-payment check on live
  staging, and a real-device mobile check of the scroll fix — both blocked on the redeploy above
  landing first.
...
- **Broader service coverage — still thin, though the specific slug bug is now fixed.** `CategoriesServiceTests` (1 test) and `ServicesServiceTests` (now 3, up from 2) remain the thinnest tests in the suite. The double-hyphen `Slugify` bug Section 3m originally flagged (`Walton-on-Thames & Weybridge` yielding a double hyphen) is fixed — see Section 3y — with a regression test covering it, but general service-layer coverage beyond that one fix is still lower than the rest of the suite.
