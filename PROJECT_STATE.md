...
2. **Real identity / NAP** (Tier 3 item 18): one real phone everywhere (navbar ×3, footer, `Services/Details` ×2, JSON-LD ×3, the FAQ's `+1555HANDYFIX`); `sameAs` removed until real profiles exist; ~~the real domain in JSON-LD and `robots.txt`~~ (done, Section 3bh); the company email in ~~Terms, Privacy~~ (done, Section 3bh), the Contact page and the footer. **Blocked on Tier 2 item 9** (real phone, trading address) — the rest of L1 doesn't wait for it.
3. **Footer**: "across Surrey and South London", company email. ~~New brand and copyright line~~ done (Section 3bh).
4. **Contact page**: replace the three generic info tiles (Expert Support / Quick Response / Local Dispatch) with phone, email, and areas/hours.
5. **FAQ**: remove the search bar and its filter script; "Chat Now" → "Send an Enquiry" (`/Contact`); the call button gets the real number.
6. **Category page SEO block**: the map placeholder and its fabricated "3 Active Technicians Nearby" are **done (Section 3be)**, replaced by the real coverage diagram, with the text now full width. **Still open**: the paragraph becomes brand-correct and driven by `Model.LocalAreas` instead of the hardcoded "Cobham, Esher, and Epsom"; do it with the rename (item 1). The area badge links are the SEO part and they stay.
7. **Done 2026-09-23 (Section 3bk).** **Reviews page**: remove `4.9` / `2.4k Verified Reviews` (Tier 3 item 16's last instance).
8. **Pricing note box** (`Pricing.cshtml` ~line 74): add "after the first hour, additional time is billed in 30-minute blocks at the same hourly rate"; bold "Plumbing and Handyman" and "Small Building & Refurbishment". Same sentence in FAQ and Terms.
9. **Dash sweep**: em-dashes in `Home/Index`, `Booking/Index`, `Home/Reviews` ×2, the four `&mdash;` entities, `PaymentsService`'s confirmation email, `HomeController`'s meta description → a single hyphen or a split sentence. Select placeholders unchanged.
