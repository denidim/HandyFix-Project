...
1. **Visible rename to "Plumbing Handyman Surrey"** (Tier 2 item 10's follow-through). Scope: every public and admin view (`Handy Fix`/`HandyFix` in navbar, footer, hero copy, `_AdminLayout`, Identity pages); every `ViewData["Title"]`/`MetaDescription` in `HomeController`, `ServicesController`, `AreasController`, `BookingController`; the three JSON-LD blocks (`name`, `url`, `@id`); all four email templates and sender display names in `BookingsService`/`PaymentsService`; the Stripe product name; `robots.txt`'s sitemap URL; `README.md`. **Not in scope**: namespaces, `.csproj`/`.sln`, Dockerfile, workflows, `handyfix_*` databases, the GHCR image, the repo name (Section 3bb decision).
2. **Real identity / NAP** (Tier 3 item 18): one real phone everywhere (navbar ×3, footer, `Services/Details` ×2, JSON-LD ×3, the FAQ's `+1555HANDYFIX`); `sameAs` removed until real profiles exist; the real domain in JSON-LD and `robots.txt`; the company email in Terms, Privacy, the Contact page and the footer. **Blocked on Tier 2 item 9** (real phone, trading address) — the rest of L1 doesn't wait for it.
3. **Footer**: "across Surrey and South London", company email, new brand and copyright line.
...
10. **Copy contradictions**: home widget "cancel anytime" → matches the 24-hour policy; `Details.cshtml` "Sutton and surrounding areas" → a generic areas line; `Booking/Index` title and `BookingController` meta → Surrey-first, new brand.
11. **Stripe wording**: kept (payments stay on Stripe); only brand-check the surrounding sentences during the rename.
...
3. **Brevo**: account, sender-domain verification (SPF/DKIM records), `Brevo:ApiKey` on staging first, then production; `Email:BookingsFromAddress` and `Email:SystemFromAddress` = `bookings@`, `Admin:NotificationEmail` = `info@`, both on the new domain, as env vars only (never in a tracked file). The code-level defaults still on `handyfix.co.uk` move to the new domain as part of L1 item 1.
