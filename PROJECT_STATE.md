...
- **Domain**: bought (`plumbing-handyman-surrey.co.uk` + `.com`, Section 3ah) but not yet pointed anywhere. Decided 2026-09-21 (Section 3bb): DNS moves to Cloudflare with the proxy on, then an A record to the production box, with Zoho MX and Brevo SPF/DKIM records alongside — Launch Sprint L4 items 1–3 in Section 4. Staging keeps running on Hetzner's free reverse-DNS hostname until then.
...
- **Real photo, not a placeholder**: the user's supplied `zap-photo.avif` (425×650; Zapryan's real photograph — this entry originally called it "AI-generated", which was wrong, corrected 2026-09-21 on the user's confirmation) had to be converted — GDI+ (`System.Drawing`) cannot decode AVIF at all ("Out of memory" is its generic unrecognized-format error), but Windows' WIC codec stack could (this machine has an AVIF codec extension installed), so a PowerShell script decoded it via `System.Windows.Media.Imaging.BitmapDecoder` to PNG, then a throwaway console app referencing this repo's own `SkiaSharp` 2.88.9 (already a dependency, used identically in `ImageStorageService`) resized it to 320px wide and re-encoded as WEBP at quality 80 — matching both the format and quality setting every other site image already uses. Saved as `wwwroot/images/expert-zap.webp` (42.5 KB), same naming convention as `expert-david.webp`/`expert-mark.webp`. The existing `onerror` fallback to `/images/hero.webp` was left in place unchanged.
...
## 3bb. Launch Planning: Audit Against the Launch Task List, Roadmap Reorganised into Launch Sprints (2026-09-21)

No code changed. The user brought a task list from a walk-through of the staging site and asked for
it to be checked against the code and the open roadmap, merged with anything still hidden in earlier
sections, and split into the final sprints before launch. The result is the **Launch Sprints**
subsection at the top of Section 4; this entry records what the audit found and the decisions made.

**Audit findings** (checked against the code on 2026-09-21, not assumed):
- `origin/dev` and `origin/main` are identical (PR #16 merged); working tree clean; last recorded suite 158/158.
- **Enquiries send no email.** `HomeController.Contact`/`JoinTeam` POST → `InquiriesService.CreateInquiryAsync` → database only. The only outbound admin notification in the codebase is the deposit-paid notice in `PaymentsService`, sent to `Admin:NotificationEmail` (default `admin@handyfix.co.uk`). Customer emails go out on booking creation (pay-the-deposit prompt), on deposit paid, and on admin approval.
- **No abuse protection of any kind**: no rate limiting, no honeypot, no captcha. `AutoValidateAntiforgeryTokenAttribute` is on globally, which stops naive cross-site posts but not a script that loads the form first.
- **Validation is attribute-level only**: `[Phone]`/`[EmailAddress]` accept nearly anything; no postcode check; no `::placeholder` rule exists anywhere in `wwwroot/css/`, so placeholder and typed text look alike.
- **Services are ordered by `Name` everywhere** (`ServicesService.GetAllAsync`/`GetByCategoryAsync`); `HomeController.Index` takes the first four of that list as "Popular Services". Nothing curates the order — "popular" is an accident of the alphabet.
- **`Booking/Index`**: the home widget lists all three categories, but the wizard's category radios only cover Plumbing/Handyman, so `?categorySlug=small-building-works` arrives with no radio checked and an empty service list. The JS "No availability on this date" message and the "Can't find your timeslot" card have no enquiry link. The base-rates card has no pricing link.
- **`Areas/Details` "Popular Services"** cards don't pass `ImageUrl` to `_PricingCard` (the only call site that doesn't) and hardcode a plumbing/handyman icon. Their "Book Now" link is also broken (added 2026-09-21, reported by the user): it passes `serviceId`, but the booking action binds `selectedServiceId`, so the wizard doesn't show the service the visitor came from; `Services/Pricing.cshtml` has the same mistake (L2 item 7).
- **`Services/Details` below 992px** stacks the sidebar CTA after the overview, FAQ and expert block; the floating mobile "Book Now" (`_MobileStickyCta`) links to the generic `/Booking`, not the service.
- **Public registration is open**: `/Identity/Account/Register` works (creates a role-less user) and is linked from `_LoginPartial` and the Login page. Not in any roadmap item until now.
- **Production has no capacity source**: `DevelopmentCapacitySeeder` is non-production only, so slots exist in production only if an admin generates them — the same mechanism behind the staging "No availability" incident in Section 3ah.
- **`expert-zap.webp` is Zapryan's real photograph.** Section 3ap described it as AI-generated; corrected there.
- **Copy leftovers not previously logged**: the home widget's "No risk — cancel anytime" contradicts the 24-hour forfeiture in Terms; `Category.cshtml`'s SEO paragraph hardcodes "Cobham, Esher, and Epsom" for every category; `Details.cshtml`'s sidebar says "available in Sutton and surrounding areas" for every service; `Booking/Index`'s `<title>` is still "HandyFix London"; the FAQ call button uses `tel:+1555HANDYFIX`.
- **Admin cancel** (`BookingsService.CancelBookingAsync`) releases the slot but sends no customer email; no refund code exists anywhere (`CancelPaymentAsync` only cancels a still-pending checkout session).

**Decisions made in the session** (business reasoning mirrored in `docs/private/VISION_AND_CONTEXT.md` Section 5.15):
- **Rename is visible text only.** Namespaces, project/solution names, Dockerfile, workflows, `handyfix_*` databases, the GHCR image and the GitHub repo stay `HandyFix` as the internal codename. Renaming identifiers touches every file and every deploy secret for no visitor-facing gain right before launch.
- **No short brand form.** The full "Plumbing Handyman Surrey" is used everywhere; in the navbar it wraps to two lines below 768px if it doesn't fit. An abbreviation would throw away the literal-name advantage that drove the choice (Section 3ah).
- **Billing increments: 30 minutes.** After the first hour, extra time is billed in 30-minute blocks at the same hourly rate. Goes into Pricing, FAQ and Terms.
- **Dashes**: every em-dash in visitor-facing text becomes a single hyphen (or the sentence is split); the `-- Choose a service --` style select placeholders are left alone.
- **Payments stay on Stripe.** The "bank details only" alternative was dropped: without a payment step the whole booking flow changes (the stale-booking sweep abandons unpaid bookings after 15 minutes, slots would need manual confirmation, Terms would change). Creating the Stripe account is a user to-do (L4 item 4), not engineering. Existing "Stripe" wording stays.
- **Cancellation at launch**: customers cancel by phone or email; admin cancels in the panel; the deposit is refunded manually in Stripe; Terms and FAQ say exactly that. Policy stays £50 deposit, free more than 24 hours out, forfeited inside 24 hours. Self-service cancel links are post-launch.
- **Category card mobile image**: deliberately unchanged for now; re-judged after the image batch (L5).
- **Font scale**: body 18px, labels 16px, nothing below 14px on public pages, headlines one step up.
- **Turnstile at launch**, on top of honeypot and rate limiting.
- **Address helper**: free postcode validation (format check plus a served-area check), no paid autocomplete.
- **DNS moves to Cloudflare** (proxy on) rather than staying at `names.co.uk`: free DDoS/bot protection, and R2 already lives there. Zoho MX and Brevo SPF/DKIM records go in Cloudflare.
- **Slots are generated only by an admin.** No automatic capacity job. The operating rule is documented in `docs/WORKFLOW_BOOKINGS.md` (L4 item 7).
- **All service images are regenerated**, including the 16 keepers with the garbled old logo.
- **Trustpilot** waits until Google reviews are collecting (vision doc Section 4).

**Answered the same day, after being re-asked**:
1. The new generic Handyman service is **"General Handyman Call-Out"** (the Plumbing equivalent is the existing "General Plumbing Maintenance"). The home page's popular four are **Full Bathroom Refurbishment, Kitchen Fitting & Alterations, Emergency Plumbing, Furniture Assembly** — the user's pick, not the suggested plumbing/handyman-only set. Two are quote-first Small Building services, which the bento card must present as such (L2 item 1).
2. Three mailboxes on `plumbing-handyman-surrey.co.uk`: `info@`, `bookings@`, and a personal one for the owner (roles in L4 item 2; the exact personal address lives only in the private vision doc, Section 5.15). Every notification goes to `info@` — one inbox to watch — and customers get an automatic acknowledgement email for enquiries and applications (L3 item 6).

Nothing to verify: documentation only. `PROJECT_STATE.md` (this entry, the header, Section 1's domain bullet, Section 3ap's photo correction, Section 4's new Launch Sprints) and the private vision doc (Section 5.15, Section 6) changed; no build or test run was needed.

---

## 3bc. Branding/Image-Gen Pipeline Built, Division Colour System Site-Wide, Category Page Background Prototyped (2026-09-22)

L5 (Images) started early, out of order — a branding strategy session, not a scheduling change; the asset batch itself (L5 item 2) has not started. Long session, closing it out mid-stream to keep context manageable; picks up directly where this leaves off.

**Style locked** *(since replaced by the bright style, Section 3bd)*: soft watercolour/pastel gouache — visible brush strokes, paper texture, muted **neutral** palette. Division colour is explicitly **not** baked into the artwork (learned the hard way once already, Section 3ai) — it's applied by CSS on top, so it can be retuned without regenerating anything. People are **painterly-but-representational**: recognisable as a specific person, not abstract. Exception: **Zapryan's real photo (`expert-zap.webp`) is left untouched** — a randomly-generated painted face doesn't solve anything for a real, named person; repainting him needs his actual photo fed in as a reference, not attempted this session.

**Model: Nano Banana Pro** (`gemini-3-pro-image-preview`), not the Section 3ai Flash model. Reason: it accepts up to 14 reference images per call (3 as dedicated style references), which is the actual fix for Flash's drift problem across a ~58-image batch — style-lock a handful of images, then feed them back in on every later call. Costs roughly 2x Flash (~£0.12/image observed for six 1K-2K images against the user's real AI Studio balance, £9.18 → £8.45), still cheap in absolute terms. The API key in use authenticates fine but doesn't look like a standard `AIzaSy...` AI Studio key (starts `AQ.`, and its model list includes `antigravity-preview-*` entries) — it's very likely Antigravity-issued rather than a permanent AI Studio key, so it may be short-lived; get a real AI Studio key before the full batch if this one stops working.

**Pipeline is now committed to the repo**, at `tools/image-gen/` — this **reverses** L5 item 1 below, which said "script outside the repo," on the user's explicit instruction this session. `.env` (the real key) and `node_modules/`/`output/` are gitignored (`tools/image-gen/.gitignore`); `.env.example` is the tracked empty placeholder. Confirmed via `git add --dry-run` that no secret would stage. Contains `generate-phase0.mjs` (calls the model, saves raw output) and `convert.mjs` (sharp → WebP q80 at the real per-convention paths).

**Phase 0 generated, approved, and already live** (not just a mockup — these are the real files in `wwwroot/images/`): `hero.webp`, `services/tap-repairs-hero.webp`, `services/shelf-installation-hero.webp`, `services/wall-floor-tiling-hero.webp` (new — this service had no image before), `areas/chessington-hero.webp` (new — the `areas/` directory didn't exist before this session; first real area hero). All five were replaced by the bright versions in Section 3bd.

**Division colour identity system, site-wide** — new tokens in `base/variables.css`: `--division-plumbing/-handyman/-building` (ink colours, matching the site's existing sky/rose/amber), plus `--division-*-wash` (gradient) and `--division-*-border`, so the one set of real values backs every consumer instead of being copy-pasted:

- `.division-tint` (`components/cards.css`) — a `mix-blend-mode: color` layer directly on an image. Tried everywhere at first (0.55 opacity read as a near-monochrome duotone filter, dialed to 0.28), then **removed everywhere except the Specialised Divisions tiles** (`.division-card .img-container`, both Home/Index and Services/Index) and the home hero's diagonal gradient (`home.css`'s `.hero-bg-overlay`) — direct user call: photos elsewhere should stay untinted, full colour.
- Everywhere else, the identity moved to the **card background** instead of the image: `.service-horizontal-card-plumbing/-handyman/-building` (category list pages), `.pricing-card-plumbing/-handyman/-building` (Pricing "Typical Job Costs", Areas/Details "Popular Services in {Area}", Details "You Might Also Need" — all three `_PricingCard` call sites, via the new `PricingCardViewModel.DivisionClass` property), and `.bento-badge-plumbing/-handyman/-building` (Home "Popular Services" — badge only, since that card has no separate box, just the photo with text overlaid).
- Bug fixed along the way: Areas/Details' `_PricingCard` call never set `ImageUrl` at all — that section had no photos and no division colour prior to this session, not something this session broke.

**Code quality pass done and verified**: full solution build 0 errors/0 new warnings, full test suite 158/158, twice (before and after a cleanup pass that centralised three copies of the same raw gradient hex into the new tokens above, removed z-index left over from a removed tint layer, and fixed two comments that had gone stale mid-session).

**Services/Category page background — decided, final.** All three category pages (Plumbing/Handyman/Small Building — one shared view) show a real painted image as the page background, via `ViewData["BodyClass"] = "page-bg-services-category"` and one CSS rule (`base/layout.css`). Real trial and error worth recording so it isn't repeated:

- `background-size: cover` (single image, scaled to fill) stretches by a *different* amount on every page, because `cover` sizes against that page's own full scrollable height — a page with more service cards is taller, so it stretches the same image more. Confirmed by direct comparison: Plumbing (13 services, taller) visibly more stretched than Small Building (8 services) with the identical image and CSS.
- `background-attachment: fixed` (sizes against the viewport instead) was tried as a fix for that — it does make every page consistent, but the image then has a fixed rendered height and doesn't repeat, so any page longer than that just runs out of image partway down and shows flat background for the rest. Rejected by the user for exactly this reason after seeing it live.
- **What's live**: `background-size: 100% auto` (scales by width only, height follows at the same ratio — zero distortion, unlike `cover`) plus `background-repeat: repeat-y` (tiles down however far the page goes, so no page-length runs out of coverage). The image, `wwwroot/images/bg-services-category.webp`, was specifically generated to **tile seamlessly** — its bottom edge matches its top edge (both land on blue) — so the repeat reads as one continuous flow rather than a visible seam. This combination is the one that held up under scrutiny.
- Provenance: the file started as a user-supplied reference image (`bg-transition.jpg`), not something generated through the `tools/image-gen/` pipeline with the locked style/prompt. It was first added under a `test-`-prefixed name (gitignored by an existing repo-wide `**/wwwroot/images/**/test*` rule, so it never actually reached staging on first attempt — caught when the user checked staging and it was missing), then renamed to its real, tracked name once confirmed as the final choice.

**Explicitly not done, next up** (done since, Section 3bd): the images generated so far read as **too dark/autumnal** for a year-round site — brighter, more colourful regeneration is the very next step, prompt work not started. Then the real L5 item 2 batch (15 area heroes, ~27 remaining Plumbing/Handyman services, 8 Small Building services, both category tiles). Zapryan's photo stays as-is until a decision is made on how to handle it.

**Committed**, five commits on `dev` (`feat(image-gen)`, `feat(images)`, `feat(ui)`, `feat(services)`, `docs(state)`), plus one follow-up making the category background permanent. Pushed to `origin/dev` (auto-deployed to staging). Merged to `main` via PR #18.

**Practical note for restarting the app locally**: the dev SQL container needs a manual `docker ps` check (Docker Desktop isn't always already running) and the connection string needs `Server=127.0.0.1,1433` rather than `localhost,1433` — the latter throws a Kerberos/SSPI error (`Cannot generate SSPI context`) in this environment despite being SQL-auth, not Windows-auth. Working command: `ConnectionStrings__DefaultConnection="Server=127.0.0.1,1433;Database=HandyFix;User Id=sa;Password=mssqlDB1;TrustServerCertificate=True;Encrypt=False;MultipleActiveResultSets=true" dotnet run --no-launch-profile --urls "http://localhost:5250"` from `src/Web/HandyFix.Web`.

### Verified

- `dotnet build HandyFix.sln`: 0 errors, 0 new warnings (both before and after the cleanup pass).
- `dotnet test HandyFix.sln`: 158/158 passing (97 `HandyFix.Services.Data.Tests` + 61 `HandyFix.Web.Tests`), both runs.
- Every changed page screenshotted live (headless Chrome) after each change, not just reasoned about from the CSS.

---

## 3bd. Bright Image Regeneration: All 59 Site Images (2026-09-22)

The Section 3bc images read too dark and nostalgic, so every image on the site was regenerated.

- **Style**: bright, saturated watercolour in direct sunlight (replaces Section 3bc's muted palette). The one fixed brand element is a **plain navy polo, no text or logo**. People vary on purpose: no recurring "technician" character, since the only real person on the site is Zaprqn. Area heroes cover all four seasons, always in bright light, each built around a real local landmark. No text, logos, signage or vehicles in any image; a logo, if ever needed for social/ads, goes on top at export time, never into the art.
- **Pipeline** (`tools/image-gen/`): `batch-jobs.mjs` (one entry per image), `generate-batch.mjs` (Nano Banana Pro at 2K, two approved images sent as style references), `review-sheet.mjs`, `convert-batch.mjs` (WebP quality 70 at effort 6: about 30% smaller than q80, no visible difference), `generate-style-test.mjs` (the 8-image test the references came from).
- **Rule: generate in batches of about five and check every image before the next batch.** The first full run went out as one 53-image job and copied the reference image's face into about 15 images before anyone looked; those were redone with a specific, different person written into each scene.
- **Shipped**: 4 page backgrounds, 3 category tiles, 37 service images, 15 area heroes (28 replaced, 31 new). `general-handyman-call-out-hero.webp` assumes that slug for the service L2 item 2 adds, so the service must use it. One hand retouch: fake letters blurred out of a spirit level in the washing-machine image.
- **Stale images fixed**: `Program.cs` now sends `Cache-Control: no-cache` for `/images/*`. Images are replaced under the same names, and without the header browsers kept showing old copies (spotted on the service pages' "You Might Also Need" cards). A crawl of all 69 public pages confirmed every image link points at an existing, new file.
- **Cleanup**: the leftover `test-image-from-admin-edit` service (Section 3j) soft-deleted in the local dev database and its image removed; old raw files deleted from the gitignored `output/`.
- **Not done**: no visual page check (the crawl checks links only), so text readability over `services-hero`, `areas-hero` and `bg-cta-wide`, and the `.division-tint` layer against the brighter art, are still to judge. `hero.webp` is 329KB with no mobile variant. Both are L5 item 3. *(Since then the user checked the text: it reads fine. `srcset` is deferred until after launch.)*

### Verified

- Every image reviewed before the next batch was generated.
- `dotnet test src/HandyFix.sln`: 159/159 (97 + 62), including a new `WebTests` check for the image cache header. Build 0 errors, no new warnings.

---

## 3be. Category Pages: Real Coverage Map Instead of the Placeholder (2026-09-22)

- The SEO block on the three category pages showed a grey map-icon placeholder captioned "3 Active Technicians Nearby", a made-up figure (Tier 3 item 16's last Category instance). It now shows the real coverage diagram from the Areas page (the shared `_AreaCoverageMap` partial). Nothing was generated: image models draw maps badly (Section 3l).
- The block is one column at every width now: the diagram is 780px wide, and in a half-width column its town labels shrink below a readable size. Phones get the partial's grouped town list, as on the Areas page.
- `ServicesController.Category` passes all areas so the diagram plots every town; the view keeps the badge links at 12, as before. The placeholder's CSS is deleted.
- Still open (L1 item 6): the paragraph's hardcoded "Handy Fix" and "Cobham, Esher, and Epsom", left for the rename.

### Verified

- `dotnet test src/HandyFix.sln`: 160/160 (97 + 63), including a new `WebTests` check that a category page has the diagram and no "Active Technicians" text. No new warnings.
...
## 3bh. Visible Rename to "Plumbing Handyman Surrey" (2026-09-22)

L1 item 1, the first L1 item. Visible text only, as decided in Section 3bb: namespaces, the solution, databases, the GHCR image and the repo keep `HandyFix` as the internal codename (the README now says so in one line).

- **One source for the name**: `GlobalConstants.SystemName` is now "Plumbing Handyman Surrey". The layout already appended it to every `<title>` and `og:title`, and the navbar logo already read it; the footer, login page and admin sidebar read it now too.
- **Page titles no longer carry the brand themselves.** Most titles had it typed in as well, so tabs read "Pricing - HandyFix Surrey & South London - HandyFix". Every title is now just the page ("Pricing", "Tap Repairs", "Handyman & Plumbing in Cobham") and the layout adds " - Plumbing Handyman Surrey" once. Meta descriptions say the new name. The Booking page title and meta are Surrey-first (part of L1 item 10).
- **Structured data**: `name` in all three JSON-LD blocks; `url`/`@id` on the home page → `https://plumbing-handyman-surrey.co.uk`. Phone, address and `sameAs` untouched: that is L1 item 2.
...
- **Found, left for the honest-copy items**: "Satisfaction Guaranteed" badge on the home page CTA and "100% satisfaction guarantee" in the service page FAQ (confirmed not real, Tier 3 item 16); ~~"thousands of homeowners" on Reviews~~ (replaced, Section 3bk); "rigorous background checks and technical vetting" on Home (unconfirmed). Privacy and Terms repeat the full name awkwardly; they're rewritten in L1 item 12.
...
## 3bk. Reviews Page: Made-Up Rating Figures Deleted (2026-09-23)

L1 item 7. The "4.9 Global Satisfaction" and "2.4k Verified Reviews" boxes on `/Reviews` sat inside `@if (Model.ShowOnSiteReviews)`, and `Business:ShowOnSiteReviews` is false in `appsettings.json` and set nowhere else (staging included), so no visitor has seen them since Section 3n. Deleted anyway, with their now-unused CSS (`.stat-overview-*` in `pages-info.css`): they would have reappeared the day on-site reviews are switched on. The page's intro line, which is visible, said "Discover why thousands of homeowners trust... Verified reviews from our community": neither claim is true yet, so it now reads "Read what homeowners across Surrey and South London say about our work."

### Verified

- `dotnet build` clean; `dotnet test src/HandyFix.sln`: 178/178.

---

## 3bl. L1 Copy Batch: Footer, Contact, FAQ, Category Text, Billing Rule, Dashes, Legal Pages (2026-09-23)

The rest of L1 that doesn't need Zaprqn, done in one pass at the user's request (items 3, 4, 5, 6, 8, 9, 10 and 12, plus L2 item 9, whose cancellation wording overlaps the FAQ and Terms). Only real facts were used: the £50 deposit and 24-hour rule and phone/email cancellation from Section 3bb, the 30-minute blocks, the Mon-Sat 9:00-17:00 hours already in the home page's structured data, the Hetzner EU (Finland) hosting from `docs/private/INFRASTRUCTURE.md`, and the form fields from the input models.

- **Footer**: Surrey first; `info@` shown as text and as the mail icon, which replaces a share icon that linked to `#`.
- **Contact tiles**: Call Us (the site-wide placeholder number), Email Us (`info@`, plus the "Replies in < 12 hrs" line from Section 3bg), Surrey & South London (Mon to Sat, 9am to 5pm). The long address wraps inside its tile (`.contact-info-text`); tiles are equal height.
- **FAQ**: search bar, its filter script and `.faq-search-input` removed; "Chat Now" → "Send an Enquiry"; `tel:+1555HANDYFIX` → the site-wide placeholder; cancellation "through your dashboard" → call or email; a new "How is my time charged?" answer with the 30-minute rule; the user's two corrections: booking confirmation is by **email only** (the SMS claim is gone) and the "support team available 24/7" line is replaced. The two buttons stack on phones, where "Send an Enquiry" wrapped to three lines.
- **Category paragraph**: names the first three real areas (featured first) from `Model.LocalAreas` instead of the hardcoded "Cobham, Esher, and Epsom", and drops "fastest in the region" and the "latest diagnostic tools" vans.
- **Billing rule**: Pricing note box (with **Plumbing and Handyman** and **Small Building & Refurbishment** in bold), the Pricing FAQ, the site FAQ and Terms all say extra time is billed in 30-minute blocks.
- **Dashes**: every visitor-facing em-dash is now two sentences: Booking, Reviews (two), Pricing (two), FAQ, the coverage-map caption and the deposit-paid email. Admin-only pages keep theirs.
- **Contradictions**: the home booking widget's "No risk, cancel anytime" → "Free cancellation up to 24 hours before your visit"; the service sidebar's "available in Sutton and surrounding areas" → "Local technicians covering Surrey and South London".
- **Legal pages** (dated 23 September 2026): **Terms** drop Kent (never served), add a Prices section, the 15-minute deposit window, cancellation by phone or email with a refund to the card, what happens if we cancel, "your statutory rights are not affected" and England and Wales law; "professional" → "public" liability insurance (Section 3ah). **Privacy** replaces the made-up US address and "+1 (555)" number, lists exactly what each form collects, says no analytics or tracking, names the processors (Stripe, Brevo, Cloudflare for photos, Hetzner in the EU, Google Fonts), sets retention (bookings and payments 6 years for tax, enquiries 12 months, applications 6 months, photos with their record), and lists rights with the ICO. **Cookie Policy** lists the four cookies the site actually sets (consent, antiforgery, TempData, staff sign-in), all essential, and explains that Stripe's cookies live on Stripe's checkout page. The **cookie banner** no longer claims analytics and links to the Cookie Policy.
- **Booking confirmed page** (L2 item 9): a fourth step, "Need to cancel or reschedule? Call or email us. It's free more than 24 hours before your visit."
- **Still open in item 12**: the legal entity name and address (Tier 2 item 9). The data controller is named by trading name only. The privacy page promises deletion on request, which the admin panel can't do yet for enquiries: L3 item 8.
...
## 3bp. Booking Page: Enquiry Button When No Slot Suits (2026-09-24)

L2 item 4. On a day with no free slot the booking page showed a red "No availability on this date" line and, lower down, the "Can't find your convenient timeslot" card, which asked visitors to leave an enquiry but gave them no way to.

- **Card left where it is** (user's call): moving it into the slots column, under the red line, was mocked up; it looked right on a day with no slots but crowded the column on a day with eight. So it keeps its full-width place under the calendar and slots, on every date.
- **`Booking/Index.cshtml`**: the card gets a **Send an Enquiry** button (`btn-primary`) and an "or call 07123 456789" link (the site-wide placeholder number). The page script keeps the button's link on `/Contact?service={name}&date=yyyy-MM-dd`, refreshed whenever the service or the day changes; the day only goes with a service, since the message names both.
- **`HomeController.Contact`** takes an optional `date`. With a service and a date in `yyyy-MM-dd` it pre-fills "Hi, I'd like to book {service} on Sun 27 Sep, but I couldn't find a time that works in the booking calendar. Could you fit me in?" Worded that way, not "no free slots", because the card shows on days that have slots too. Any other date is ignored and the existing quote message is used, so the building pages' quote links are unchanged. The date is formatted with the invariant culture: en-GB would print "Sept".
- **Styles** (`booking.css`): `.reassuring-info-actions`, the button's icon size, the call link.
- **Noticed, not changed**: the Contact page heading still says "Get a Custom Quote", which reads a little oddly for a booking enquiry. Switching to Handyman picks "Bath & Shower Screen Fitting" first, since there is no general handyman service yet (L2 item 2).

### Verified

- New `HomeControllerTests`: a service and date give the booking message with the category preselected; `not-a-date`, `27/09/2026` and `2026-02-30` fall back to the quote message.
- Local run in Chromium at 1440px and 390px: on Sun 27 Sep (no slots) the link is `/Contact?service=General+Plumbing+Maintenance&date=2026-09-27`; picking the 28th changes it to that day; switching to Handyman changes the service (an "&" in the name arrives encoded); following it opens Contact with the message above and Plumbing selected. The card is in its old place on both widths. No console errors.
- `dotnet build` clean (HomeController.cs's SA1412 BOM warning predates this); `dotnet test src/HandyFix.sln`: 198/198 (97 + 101).
...
> **This is the day-to-day plan.** It supersedes the tiered list further down as the thing to work from; the tier items are kept for their history and reasoning, and the mapping table under "Pre-Sprint 4 TODOs" says where each one went. Rules: one sprint at a time, in order; items marked *** are explicitly **not launch-blocking** and sit last in their sprint; external accounts with lead time (Google Business Profile, Cloudflare, Zoho, Brevo, Stripe) are opened during L1 even though the code that uses them lands in L4; every item is checked live on staging before its sprint closes; `PROJECT_STATE.md` gets a Section 3 entry per session as usual. Why this order: the rename comes first because every later copy change would otherwise be written twice; the flow work second; abuse protection third so it validates forms that have stopped moving; infrastructure after that because DNS, email and payments all wait on the rename to settle the addresses; images last by the user's choice.

#### L1 — Identity and honest copy

1. **Done 2026-09-22 (Section 3bh).** **Visible rename to "Plumbing Handyman Surrey"** (Tier 2 item 10's follow-through). Scope: every public and admin view (`Handy Fix`/`HandyFix` in navbar, footer, hero copy, `_AdminLayout`, Identity pages); every `ViewData["Title"]`/`MetaDescription` in `HomeController`, `ServicesController`, `AreasController`, `BookingController`; the three JSON-LD blocks (`name`, `url`, `@id`); all four email templates and sender display names in `BookingsService`/`PaymentsService`; the Stripe product name; `robots.txt`'s sitemap URL; `README.md`. **Not in scope**: namespaces, `.csproj`/`.sln`, Dockerfile, workflows, `handyfix_*` databases, the GHCR image, the repo name (Section 3bb decision).
2. **Real identity / NAP** (Tier 3 item 18): one real phone everywhere (navbar ×3, footer, `Services/Details` ×2, JSON-LD ×3, ~~the FAQ's `+1555HANDYFIX`~~ now the same placeholder as everywhere else, Section 3bl); `sameAs` removed until real profiles exist; ~~the real domain in JSON-LD and `robots.txt`~~ (done, Section 3bh); ~~the company email in Terms, Privacy, the Contact page and the footer~~ (done, Sections 3bh and 3bl). **Blocked on Tier 2 item 9** (real phone, trading address) — the rest of L1 doesn't wait for it.
3. **Done 2026-09-23 (Section 3bl).** **Footer**: "across Surrey and South London", company email; new brand and copyright line (Section 3bh).
4. **Done 2026-09-23 (Section 3bl).** **Contact page**: replace the three generic info tiles (Expert Support / Quick Response / Local Dispatch) with phone, email, and areas/hours.
5. **Done 2026-09-23 (Section 3bl).** **FAQ**: remove the search bar and its filter script; "Chat Now" → "Send an Enquiry" (`/Contact`); the call button gets the real number (the site-wide placeholder until Tier 2 item 9 lands, same as item 2).
6. **Done 2026-09-23 (Section 3bl).** **Category page SEO block**: the map placeholder and its fabricated "3 Active Technicians Nearby" are **done (Section 3be)**, replaced by the real coverage diagram, with the text now full width. ~~Still open: the paragraph becomes brand-correct and driven by `Model.LocalAreas` instead of the hardcoded "Cobham, Esher, and Epsom"~~ done, Section 3bl. The area badge links are the SEO part and they stay.
7. **Done 2026-09-23 (Section 3bk).** **Reviews page**: remove `4.9` / `2.4k Verified Reviews` (Tier 3 item 16's last instance).
8. **Done 2026-09-23 (Section 3bl).** **Pricing note box** (`Pricing.cshtml` ~line 74): add "after the first hour, additional time is billed in 30-minute blocks at the same hourly rate"; bold "Plumbing and Handyman" and "Small Building & Refurbishment". Same sentence in FAQ and Terms.
9. **Done 2026-09-23 (Section 3bl).** **Dash sweep**: em-dashes in `Home/Index`, `Booking/Index`, `Home/Reviews` ×2, the four `&mdash;` entities, `PaymentsService`'s confirmation email, `HomeController`'s meta description → a single hyphen or a split sentence. Select placeholders unchanged.
10. **Done 2026-09-23 (Section 3bl).** **Copy contradictions**: home widget "cancel anytime" → matches the 24-hour policy; `Details.cshtml` "Sutton and surrounding areas" → a generic areas line; ~~`Booking/Index` title and `BookingController` meta → Surrey-first, new brand~~ done (Section 3bh).
...
12. **Done 2026-09-23 except the legal entity and address (Section 3bl).** **Legal pages, content half** (Tier 3 item 24): Privacy, Terms and Cookie Policy rewritten to what the site does — real dates, data controller, R2 photo storage, Brevo, job applications, retention periods, the site's own cookies listed (antiforgery, login, consent, TempData), banner text without the analytics claim, the 30-minute billing rule, the cancellation process from Section 3bb. **Blocked on Tier 2 item 9** for the legal entity and address; everything else can be drafted now.
...
4. **Done 2026-09-24 (Section 3bp)**, as a "Send an Enquiry" button and a call link in the timeslot card; the red message itself got no link, since the card sits right under it. **No-availability enquiry** (Tier 6 item 23): a "Send a quick enquiry" button under the "Can't find your timeslot" card and inside the JS "No availability on this date" message, linking to `/Contact?service={name}` with the preferred date appended to the pre-filled message (the Contact GET already pre-selects the category and pre-fills the message); plus a "Call us" link.
...
9. **Done 2026-09-23 (Section 3bl).** **Cancellation copy**: FAQ "through your dashboard" → "call or email us"; Terms section 3 restated per Section 3bb; `Booking/Confirmed` gets a "need to cancel or reschedule? call or email us" line.
...
#### L3 — Abuse protection, validation and notifications

1. **Honeypot + time-to-submit** on Contact, Join Our Team and Booking: a hidden field that must stay empty and a rendered-at timestamp; a submission under ~3 seconds or with the field filled is accepted-and-dropped (same thank-you, nothing saved) so bots learn nothing from the response.
2. **Rate limiting**: `AddRateLimiter` (built into ASP.NET Core, no package) with a fixed-window policy per client IP on the three POST endpoints and `/Booking/GetSlots`; the IP comes from the forwarded headers Caddy already supplies (`Program.cs` ForwardedHeaders block). Suggested: 5 form posts per 10 minutes per IP, a friendly 429 page.
3. **Cloudflare Turnstile** on the three forms; site key/secret via env vars; skipped in Development and in `SqliteWebApplicationFactory` tests when no key is configured, following the fail-loud-outside-development pattern in Section 5.
...
#### L4 — Infrastructure and go-live

1. **Cloudflare DNS**: move `plumbing-handyman-surrey.co.uk` (and `.com`) nameservers from `names.co.uk` to Cloudflare; A record → the production box with the proxy on; `.com` redirects to `.co.uk`. Record the real values in `docs/private/INFRASTRUCTURE.md`.
2. **Zoho Mail**: MX/SPF/DKIM in Cloudflare; three mailboxes (decided 2026-09-21): `info@` (the public address on the site; receives every notification; the one inbox Zaprqn watches), `bookings@` (the From address on every customer email, with Zoho forwarding to `info@` so replies land in the same inbox), and a personal mailbox for the owner (exact address in the private vision doc Section 5.15; it's also the login for the Google Business Profile, Stripe and Cloudflare accounts, so ownership isn't tied to anyone's private email — the vision doc Section 5.6 lesson). Two of the five free seats stay spare.
3. **Brevo**: account, sender-domain verification (SPF/DKIM records), `Brevo:ApiKey` on staging first, then production; `Email:BookingsFromAddress` and `Email:SystemFromAddress` = `bookings@`, `Admin:NotificationEmail` = `info@`, both on the new domain, as env vars only (never in a tracked file). The code-level defaults moved to the new domain in Section 3bh (L1 item 1).
4. **Stripe** (user to-do, about a day): live account, `Stripe:SecretKey` + `Stripe:WebhookSecret` in the production env; the webhook endpoint registered; `Stripe:AllowSandboxOutsideDevelopment` must not exist in production (Tier 4 item 19).
5. **Production deploy** (Tier 4 item 19 remainder): `deploy-prod.yml` triggered by `main`; a production Caddyfile (real hostname, no Basic Auth, no `X-Robots-Tag`); a production compose file; the prod `.env` on the server; the `handyfix_prod` login; rotate the DB host's `sa` password; `docs/WORKFLOW_DEPLOYMENT.md` and `docs/private/STAGING_RUNBOOK.md` updated.
6. **Data entry**: the technician roster through `/Administration/Technicians` (Tier 2 item 11 — edit "John Doe" into a real person or deactivate it); Zapryan's real details wherever L1 left a placeholder; a read-through of every service and area page.
7. **Capacity**: an admin generates the first weeks of slots before go-live; the ongoing rule (who, how often, how far ahead) is written into `docs/WORKFLOW_BOOKINGS.md`. No automatic job, by decision (Section 3bb).
8. **Google Business Profile** (Tier 3 item 13): create it and start verification during L1 week — the postcard is the longest lead time on this list; `Business:GoogleReviewsUrl` set once it exists.
9. **Staging QA pass** against a written checklist (new `docs/LAUNCH_CHECKLIST.md`): every form with real emails; booking → Stripe test mode → confirmation → admin assign → approve; admin CRUD; sitemap and robots; the 404 page; mobile at 390px; no console errors. Then PR `dev` → `main`, production deploy, smoke test on the real domain.
10. **Dev DB QA leftovers** (Tier 5 item 20): close out.
11. *** **Trustpilot**: after Google reviews are collecting.

#### L5 — Images (last, by decision)

1. **Pipeline**: superseded by Section 3bc — built and committed at `tools/image-gen/` (deliberately **in** the repo now, reversing this item's original "outside the repo" call), using Nano Banana Pro (`gemini-3-pro-image-preview`) rather than the Section 3ai Flash model, WebP quality 80, exact filenames per Tier 1 item 1's conventions below. Explicit "no text, no logos, no lettering of any kind" in every prompt, and a visual check before use — the model fabricated brand text twice in Section 3ai. Style: bright, sunlit watercolour (Section 3bd, replacing Section 3bc's muted look); division colour still applied by CSS, never baked into the art.
2. **Asset list** — **done 2026-09-22, Section 3bd** (~58 images): all 28 Plumbing/Handyman services (the 16 keepers regenerated logo-free, the 7 duplicates replaced, the 8 Section 3ak services, plus the new call-out service); the 8 Small Building services and `small-building-category-hero.webp`; both category tiles; 15 area heroes (`/images/areas/{slug}-hero.webp`, 1600×700); a new `areas-hero.webp`; ~~optionally a generic map image for the category page~~ not needed: it reuses the real coverage diagram (Section 3be).
3. **After the batch**: re-judge the category card mobile layout (L2 item 11) and the `.division-tint` layer against the brighter art. Text over the new backgrounds: checked by the user, reads fine. `srcset` mobile variants: deferred until after launch, and only if Google's speed test flags image weight (decided 2026-09-22).

---

### Pre-Sprint 4 TODOs — resequenced by launch-blocking priority (updated 2026-07-30)

> **As of 2026-09-21 the Launch Sprints above are the working list**; the items below are kept for their history and reasoning. Where they went: item 1 → L5; item 9 → still the blocker for L1 items 2/12 and L4 item 6; item 11 → L4 item 6; item 13 → L4 item 8; item 14's DNS/Zoho follow-up → L4 items 1–2 (DNS now via Cloudflare); item 15 → L1 item 13; item 16's last two instances → L1 items 6–7; item 17 → the photo is real (Section 3bb), the bind-to-entity and email-card halves are post-launch; item 18 → L1 item 2; item 19's remainder → L4 items 4–5; item 20 → L4 item 10; item 23 → L2 item 4; item 24 → L1 item 12 (content) and L3 item 8 (hard delete); items 21–22 stay post-launch.
...
**1. Image generation batch.** Bulk-generate and download images via a terminal script kept **outside the repo** — no committed generator project. The plumbing/handyman two-tone gets applied **via CSS later**, not baked into the generated artwork, so the tone can be retuned without regenerating anything.
  ...
  - **Prompting strategy**: structured/JSON-style prompts (a fixed schema per shot — subject, style, lighting, camera angle, palette, negative prompt) to keep a large batch visually consistent, using the 16 existing keeper images as style anchors (no client photos needed, confirmed 2026-09-22). Full prompt-schema design is implementation-session work, not designed yet.
  - **The agreed workflow, end to end:** generate outside the repo → **convert to WebP outside the repo** (quality 80, to match what the app's own pipeline produces; the Section 3bd batch used 70, see there) → **name each file exactly per the conventions below** → copy the finished `.webp` files straight into `wwwroot/images/`. The app is not involved at any step: no admin upload needed, no startup conversion, and no database changes, because `ServicesSeeder.cs:79` already created `ServiceImage` rows pointing at the convention path `/images/services/{slug}-hero.webp`.
...
**16. Fake statistics → real or removed.** Replace fabricated figures across `Home/Index`, `Home/Reviews`, `Home/About`, `Services/Index`, `Services/Details`, `Services/Category`:
  ...
  - **Still blocked on item 9 (real facts)**: ~~`12k+ Jobs Completed`, `4.9/5 Rating`, `Based on 2,500 reviews` (`Home/Index.cshtml:251,262,263`)~~ removed Section 3at; ~~`4.9` + `2.4k Verified Reviews` (`Home/Reviews.cshtml:96,109-113`)~~ removed Section 3bk; ~~`4.9/5 Rating` + `Over 1,200 services completed` (`Services/Details.cshtml:170,173`)~~ removed Section 3av; ~~`4.9/5 Average Rating` (`Services/Index.cshtml:95`)~~ replaced Section 3ay; ~~`5,000+ Successful Fixes`, `15+ Specialist Techs`, `Crafting Quality Since 2018` (`Home/About.cshtml:14,48-53`)~~ removed Section 3au; ~~`3 Active Technicians Nearby` (`Services/Category.cshtml:140`)~~ removed Section 3be. The counts contradict each other (12k+ vs 5,000+ vs 1,200 jobs; 2,500 vs 2.4k reviews vs 5 rows in the database), and most are rating/review claims that no longer make sense once Reviews goes GBP-link-only (item 3) — expect most to become the honest, already-identified substitutes rather than real numbers: *"Direct to your technician — no call centre"*, *"Covering 15 areas from Chessington"* (real `ServiceArea` rows), *"Fixed hourly rates, quoted upfront"* (real `BasePrice`), *"Pay securely online — deposit only"* (real Stripe integration).
...
**24. Legal pages & data protection — all of it needs proper work before launch.** *Content rewritten 2026-09-23 (Section 3bl): every gap below except the legal entity/address and the enquiry hard delete (L3 item 8) is closed. The notes below are the original audit.* Added 2026-09-10 at the user's request. The Privacy Policy, Terms & Conditions and Cookie Policy are template text that has never been checked against what the site actually does; none of it should be treated as legally sound. Known gaps, checked against the code on 2026-09-10:
...
**23. Fallback Booking Flow / Lead Generation.** **Done 2026-09-24 (Section 3bp)** through the Contact form rather than a separate waitlist form: the enquiry arrives in the admin Enquiries list with the service and day already written in. When the availability calendar has no open slots, users should not hit a dead end. Instead of "No slots available", the booking UI should fall back to a "Request a Quote" or "Join Waitlist" form. This captures the lead (name, phone, problem description) so the admin can call and manually schedule them outside standard slots. Added to roadmap following a business strategy QA session (2026-08-03).
