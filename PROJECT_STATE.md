...
>
> **Last updated**: 2026-09-22 (second session) — **L5 item 2 done: all 59 site images regenerated in a bright, sunlit watercolour style.** See Section 3bd. Generated with style references in batches of about five, every image checked before the next batch (now the rule, since each image costs real money). Browsers are now told to re-check images, so replaced pictures show up straight away. Tests 159/159. Committed on `dev` alongside this entry. Next: L5 item 3 (text readability over the brighter backgrounds, `srcset`). Previous update (2026-09-22, Section 3bc): image pipeline, division colour system, category page background.
...
- **Not done**: no visual page check (the crawl checks links only), so text readability over `services-hero`, `areas-hero` and `bg-cta-wide`, and the `.division-tint` layer against the brighter art, are still to judge. `hero.webp` is 329KB with no mobile variant. Both are L5 item 3.
...
6. **Category page SEO block**: remove `.seo-map-container` (`Category.cshtml` ~line 179) and its CSS in `service-category.css`, which also removes the fabricated "3 Active Technicians Nearby" (Tier 3 item 16's last Category instance). The text column spans full width; the paragraph becomes brand-correct and driven by `Model.LocalAreas` instead of the hardcoded "Cobham, Esher, and Epsom". A placeholder map icon has no SEO value; the area badge links beside it are the SEO part and they stay.
...
2. **Asset list** — **done 2026-09-22, Section 3bd** (all but the optional map image) (~58 images): all 28 Plumbing/Handyman services (the 16 keepers regenerated logo-free, the 7 duplicates replaced, the 8 Section 3ak services, plus the new call-out service); the 8 Small Building services and `small-building-category-hero.webp`; both category tiles; 15 area heroes (`/images/areas/{slug}-hero.webp`, 1600×700); a new `areas-hero.webp`; optionally a generic map image for the category page if the L1 removal leaves that section visually thin.
3. **After the batch**: re-judge the category card mobile layout (L2 item 11) and consider `srcset` mobile variants; also check text readability over the brighter page backgrounds and the `.division-tint` layer (Section 3bd).
