...
**Services/Category page background — prototyped, still marked `TEMPORARY` in `base/layout.css`, not a final decision.** All three category pages (Plumbing/Handyman/Small Building — one shared view) now show a real painted image as the page background, via `ViewData["BodyClass"] = "page-bg-services-category"` and one CSS rule. Real trial and error worth recording so it isn't repeated:
...
- **What's live now**: `background-size: 100% auto` (scales by width only, height follows at the same ratio — zero distortion, unlike `cover`) plus `background-repeat: repeat-y` (tiles down however far the page goes, so no page-length runs out of coverage). The image (`test-bg-vertical.webp`, sourced from a user-supplied `bg-transition.jpg`) was specifically generated to **tile seamlessly** — its bottom edge matches its top edge (both land on blue) — so the repeat reads as one continuous flow rather than a visible seam. This combination is the one that held up under scrutiny.
- `test-bg-vertical.webp` is a placeholder, not a finished brand asset — it's literally the user-uploaded reference image, not something generated through the pipeline above with the locked style/prompt. Treat the whole rule as disposable until a real decision is made.

**Explicitly not done, next up**: the images generated so far read as **too dark/autumnal** for a year-round site — brighter, more colourful regeneration is the very next step, prompt work not started. Then the real L5 item 2 batch (15 area heroes, ~27 remaining Plumbing/Handyman services, 8 Small Building services, both category tiles). Zapryan's photo stays as-is until a decision is made on how to handle it. Whether the Category page background experiment above ships, changes, or reverts is still open.

**Nothing from this entire session is committed** — everything above sits uncommitted on `dev`. A fresh session picking this up needs no special git action, just this entry.
