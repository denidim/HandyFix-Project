...
- **Not yet done** — tracked in the roadmap as Tier 4: app-level connection strings for `appsettings.Staging.json`/`appsettings.Production.json` against `<DB_PRIVATE_IP>`, `Database.Migrate()`/seeding running automatically on startup (already the pattern for local dev — just needs pointing at the new hosts), and the GitHub Actions deploy pipelines (`deploy-dev.yml` on push to `dev`, `deploy-prod.yml` on push to `main`), including a dedicated Actions SSH keypair and repo secrets (real IPs and credentials live in GitHub encrypted secrets at that point, never in a committed file).
...
**19. Hosting & CI/CD.** Infrastructure is already provisioned — see Section 1 "Hosting & Infrastructure" for the full Hetzner architecture. Remaining work, none of it blocked on business facts:
...
- **Broader service coverage — still open.** `CategoriesServiceTests` (1 test) and `ServicesServiceTests` (2) are the thinnest in the suite, and `ServicesService.Slugify` is load-bearing yet barely covered — Section 3m notes it cannot even reproduce the seeded area slugs (`Walton-on-Thames & Weybridge` yields a double hyphen). Lower risk than the controller gap, hence deferred rather than done here.
