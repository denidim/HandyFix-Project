...
>
> **Last updated**: 2026-09-21 — **Launch planning: roadmap reorganised into five Launch Sprints.** No code changed. The user's launch task list (from a walk-through of staging) was checked item by item against the code and against every still-open roadmap item, then both were folded into Launch Sprints L1–L5 at the top of Section 4: identity and honest copy; booking and enquiry flow; abuse protection, validation and notifications; infrastructure and go-live; images last. Business decisions made in the same session are recorded in Section 3bb (30-minute billing increments, cancellation by phone/email plus the admin panel, Stripe kept with the account setup as a user to-do, DNS moving to Cloudflare, Turnstile at launch, free postcode validation, slots generated only by an admin, every service image regenerated). Section 3ap's claim that the Local Expert photo is AI-generated was wrong and is corrected. The two questions left open at first (the Handyman call-out name plus the popular four, and the mailbox list) were answered the same day; see the end of Section 3bb. Previous update (2026-09-10, Sections 3av–3ba): mobile UX fixes, Join Our Team form, pricing consistency, Services index redesign. Full suite 158/158 green.
...
1. **Pipeline**: reuse the Section 3ai Gemini setup (script outside the repo, WebP quality 80, exact filenames per Tier 1 item 1's conventions). Explicit "no text, no logos, no lettering of any kind" in every prompt, and a visual check before use — the model fabricated brand text twice in Section 3ai.
