...
>
> **Last updated**: 2026-09-22 — **L5 (Images) started early: branding strategy, image-gen pipeline, and a site-wide division colour system.** See Section 3bc for the full account. Style locked (soft watercolour/pastel gouache, neutral palette, colour via CSS never baked into the art) and the pipeline is now committed at `tools/image-gen/` using Nano Banana Pro — both reverse assumptions in the original L5 item 1, updated in place. Five Phase 0 images are generated and already live in `wwwroot/images/`. A division colour-identity system now covers every service/category card site-wide (new `--division-*` tokens in `base/variables.css`). The Services/Category pages have a **decided, final** painted background (`bg-services-category.webp`, tiled seamlessly via `repeat-y`) after ruling out `cover` (stretches inconsistently per page length) and `fixed` attachment (runs out on long pages) — see Section 3bc. Full solution build 0 errors, full suite 158/158, twice. **Committed** (six commits on `dev`, pushed to `origin/dev`, staging auto-deployed; not yet merged to `main`). Next: the generated images read too dark/autumnal and need a brighter regeneration pass before the real L5 item 2 batch starts. Previous update (2026-09-21, Section 3bb): launch task list folded into Launch Sprints L1–L5.
...
**Style locked**: soft watercolour/pastel gouache — visible brush strokes, paper texture, muted **neutral** palette. Division colour is explicitly **not** baked into the artwork (learned the hard way once already, Section 3ai) — it's applied by CSS on top, so it can be retuned without regenerating anything. People are **painterly-but-representational**: recognisable as a specific person, not abstract. Exception: **Zapryan's real photo (`expert-zap.webp`) is left untouched** — a randomly-generated painted face doesn't solve anything for a real, named person; repainting him needs his actual photo fed in as a reference, not attempted this session.
...
**Explicitly not done, next up**: the images generated so far read as **too dark/autumnal** for a year-round site — brighter, more colourful regeneration is the very next step, prompt work not started. Then the real L5 item 2 batch (15 area heroes, ~27 remaining Plumbing/Handyman services, 8 Small Building services, both category tiles). Zapryan's photo stays as-is until a decision is made on how to handle it.

**Committed**, five commits on `dev` (`feat(image-gen)`, `feat(images)`, `feat(ui)`, `feat(services)`, `docs(state)`), plus one follow-up making the category background permanent. Pushed to `origin/dev` (auto-deployed to staging). Not yet merged to `main`.
...
3. **After the batch**: re-judge the category card mobile layout (L2 item 11) and consider `srcset` mobile variants.
