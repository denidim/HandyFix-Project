...
- **`Areas/Details` "Popular Services"** cards don't pass `ImageUrl` to `_PricingCard` (the only call site that doesn't) and hardcode a plumbing/handyman icon.
...
7. **Areas details cards**: pass `ImageUrl = svc.ImageUrl` and the category-correct icon into `_PricingCard` (`Areas/Details.cshtml` ~line 78) — a one-line gap, the partial already renders the image.
